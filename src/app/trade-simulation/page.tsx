...
import { useSimulationOrders } from '@/hooks/useSimulationOrders'
import { PlaceABidSimulation } from '@/components/dialogs/simulation/PlaceABid'
import { SimulationSaleDialog } from '@/components/dialogs/simulation/Sale'
...
export default function Page() {
  ...
  const { currentPrice } = useContext(SimulationFetcherContext)
  ...
  const { orders: listOrders, mutate: mutateList } = useSimulationOrders(false)
  const { orders: bidOrders, mutate: mutateBid } = useSimulationOrders(true)
  ...
  return (
    ...
    >
      ...
      <SimulationSaleDialog open={openSale} onChange={setOpenSale} selected={selected} />
      <div className={'container mx-auto text-white pt-40 pb-36'}>
        <div className='flex flex-col items-end gap-3'>
          <div>USDC Balance: {balance}</div>
          <div>Time Capsules: {quantity}</div>
        </div>
          ...
          <div>
            ...
            <div className='trade-card'>
              ...
              <div className='flex justify-center'>
                ...
                <PlaceABidSimulation open={openBid} onChange={setOpenBid} mutate={mutateBid} />
              </div>
            </div>
          </div>
          <div>
            ...
            <div className='trade-card'>
              ...
              <div className='flex justify-center'>
                ...
                <SimulationListForSale open={openList} onChange={setOpenList} mutate={mutateList} />
              </div>
            </div>
          </div>
        </div>
