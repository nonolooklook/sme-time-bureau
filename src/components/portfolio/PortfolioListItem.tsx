...
export const PortfolioListItem = ({
  ...
  const count = BigInt(isBid ? csd?.[0]?.startAmount : offer?.[0]?.startAmount)
  const min = BigInt(isBid ? offer?.[0]?.startAmount : csd?.[0]?.startAmount) / count
  const max = BigInt(isBid ? offer?.[0]?.endAmount : csd?.[0]?.endAmount) / count
